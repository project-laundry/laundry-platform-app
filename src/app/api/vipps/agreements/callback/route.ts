...
export async function GET(request: NextRequest) {
  try {
    ...
    // Verify agreement status with Vipps
    try {
      const vipps = createVippsClient();
      const agreement = await vipps.getAgreement(subscription.provider_agreement_id);

      // Check if agreement is active
      if (agreement.status !== 'ACTIVE') {
        // User may have cancelled or not approved yet
        return NextResponse.redirect(
          new URL(
            `/orders/confirm?error=agreement_not_active&status=${agreement.status}`,
            request.url
          )
        );
      }

      // Agreement is active! Redirect to success page
      // The webhook will handle payment capture and subscription activation
      return NextResponse.redirect(
