    ...
    <details className="group bg-white rounded-lg border border-gray-200 overflow-hidden">
      <summary className="flex items-center justify-between cursor-pointer list-none px-6 py-4 hover:bg-gray-50 transition-colors">
        <div className="flex items-center gap-3">
          <h3 className="text-lg font-semibold text-dark-gray">Historikk</h3>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {orders.map((order) => (
                <tr
                  key={order.id}
                  className="hover:bg-gray-50 transition-colors cursor-pointer opacity-75"
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Link href={`/orders/${order.id}`} className="text-nordic-blue hover:underline font-medium">
                      #{order.order_number}
                    </Link>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
